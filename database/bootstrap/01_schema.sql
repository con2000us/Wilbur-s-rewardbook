...
CREATE TABLE public.custom_reward_types (
    ...
    has_extra_input boolean DEFAULT false,
    extra_input_schema jsonb,
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
...
--
-- Name: COLUMN reward_rules.reward_formula; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.reward_rules.reward_formula IS 'Reward formula. Variables: G=score, P=percentage, M=max_score. Example: G*10';
...
CREATE VIEW public.student_summary AS
 SELECT id AS student_id,
    ...
    ( SELECT count(DISTINCT subjects.id) AS count
          ...
          WHERE (subjects.student_id = s.id)) AS total_subjects,
    ( SELECT count(a.id) AS count
           FROM public.assessments a
          WHERE (a.subject_id IN ( SELECT subjects.id
                   FROM public.subjects
                  WHERE (subjects.student_id = s.id)))) AS total_assessments,
    ( SELECT count(a.id) AS count
           FROM public.assessments a
          WHERE ((a.subject_id IN ( SELECT subjects.id
                   FROM public.subjects
                  WHERE (subjects.student_id = s.id))) AND (a.status = 'completed'::text))) AS completed_assessments,
    COALESCE(( SELECT sum(transactions.amount) AS sum
           FROM public.transactions
          WHERE ((transactions.student_id = s.id) AND (transactions.amount > (0)::numeric))), (0)::numeric) AS total_earned,
    COALESCE(( SELECT sum(abs(transactions.amount)) AS sum
           FROM public.transactions
          WHERE ((transactions.student_id = s.id) AND (transactions.amount < (0)::numeric))), (0)::numeric) AS total_spent,
    COALESCE(( SELECT sum(transactions.amount) AS sum
           FROM public.transactions
          WHERE (transactions.student_id = s.id)), (0)::numeric) AS balance
...
ALTER TABLE public.achievement_event_reward_rules ENABLE ROW LEVEL SECURITY;
...
--
-- Name: custom_reward_types; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.custom_reward_types ENABLE ROW LEVEL SECURITY;

--
-- Name: exchange_rules; Type: ROW SECURITY; Schema: public; Owner: -
--
