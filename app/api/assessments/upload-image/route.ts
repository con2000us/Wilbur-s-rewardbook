  ...
  try {
    ...
    if (!allowedTypes.includes(file.type)) {
      return NextResponse.json(
        { success: false, error: 'Unsupported file type. Allowed: JPEG, PNG, WebP, GIF, SVG' },
        { status: 400 }
      )
    }
    ...
    if (file.size > singleFileLimit) {
      ...
      return NextResponse.json(
        { success: false, error: `File size exceeds the limit of ${limitMB}MB per file` },
        { status: 413 }
      )
    }
    ...
    if (uploadError) {
      console.error('Assessment image upload error:', uploadError)
      return NextResponse.json(
