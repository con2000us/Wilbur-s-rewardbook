...
**Just run one file!** Execute `setup-database.sql` in your Supabase SQL Editor.

**只需執行一個文件！** 在 Supabase SQL Editor 中執行 `setup-database.sql`。
...
The `setup-database.sql` file includes all necessary migrations in the correct order:

`setup-database.sql` 文件按正確順序包含所有必要的遷移：
...
- **`subject-specific-reward-rules.sql`** - Example subject-specific rules
...
If you prefer to run migrations individually, you can use the separate `add-*.sql` files:

如果你更喜歡單獨執行遷移，可以使用單獨的 `add-*.sql` 文件：

1. `add-site-settings.sql`
2. `add-student-display-order.sql`
3. `add-subject-to-reward-rules.sql`
4. `add-reward-rules-display-order.sql`
5. `add-reset-transaction-type.sql`
6. `add-pagination-settings.sql`
7. `add-backups-table.sql`

However, **we recommend using `setup-database.sql`** for simplicity.

但是，**我們建議使用 `setup-database.sql`** 以簡化流程。
