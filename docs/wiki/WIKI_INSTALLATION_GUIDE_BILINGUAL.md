...
2. **Click the "Deploy with Vercel" button** / **點擊「Deploy with Vercel」按鈕**：
   ...
   - Or go directly to: https://vercel.com/new/clone?repository-url=https://github.com/con2000us/Wilbur-s-rewardbook&env=NEXT_PUBLIC_SUPABASE_URL,NEXT_PUBLIC_SUPABASE_ANON_KEY,SITE_PASSWORD,AI_PROVIDER_KEY_ENCRYPTION_SECRET,AI_PROVIDER_KEY_ACTIVE_VERSION
...
Before deploying, you need to set 5 environment variables:
在部署之前，需要設置 5 個環境變數：
...
5. **Add the fourth variable** (AI key encryption) / **添加第四個變數**（AI 金鑰加密）：
   - **Name** / **名稱**：`AI_PROVIDER_KEY_ENCRYPTION_SECRET`
   - **Value** / **值**：Set a long random server-only secret / 設定一個長且隨機的伺服器端密鑰
   - Click **"Add"** / 點擊「Add」

6. **Add the fifth variable** (AI key version) / **添加第五個變數**（AI 金鑰版本）：
