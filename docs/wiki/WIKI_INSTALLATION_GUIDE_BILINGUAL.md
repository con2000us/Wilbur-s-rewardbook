...
3. **Open and run these files in order** / **依序打開並執行這些檔案**：
   - `database/bootstrap/01_schema.sql`
   - `database/bootstrap/02_seed_defaults.sql`
   - Optional / 選用：`database/bootstrap/03_seed_optional.sql`
   - Find each file on the GitHub project page / 在 GitHub 專案頁面找到每個檔案
   - Click the file name, then click "Raw" / 點擊檔案名稱，再點擊「Raw」
   - Select all (Ctrl+A) and copy (Ctrl+C) all content / 全選（Ctrl+A）並複製（Ctrl+C）所有內容

4. **Paste each file into Supabase SQL Editor and run it** / **逐一貼上到 Supabase SQL Editor 並執行**：
   ...
   - After one file succeeds, run the next file / 每個檔案成功後，再執行下一個檔案
...
> 💡 **Tip** / **小提示**：`database/bootstrap/01_schema.sql` now includes large-goal tables, columns, and RLS. The Storage bucket is still a separate Supabase dashboard step.
> 💡 **小提示**：目前 `database/bootstrap/01_schema.sql` 已包含大型目標資料表、欄位與 RLS；Storage bucket 仍需另外在 Supabase 後台建立。
...
2. **Click the "Deploy with Vercel" button** / **點擊「Deploy with Vercel」按鈕**：
   ...
   - Or go directly to: https://vercel.com/new/clone?repository-url=https://github.com/con2000us/Wilbur-s-rewardbook&env=NEXT_PUBLIC_SUPABASE_URL,NEXT_PUBLIC_SUPABASE_ANON_KEY,SITE_PASSWORD,SUPABASE_SERVICE_ROLE_KEY,AI_PROVIDER_KEY_ENCRYPTION_SECRET,AI_PROVIDER_KEY_ACTIVE_VERSION
...
Before deploying, you need to set 6 environment variables:
在部署之前，需要設置 6 個環境變數：
...
5. **Add the fourth variable** (Supabase service role) / **添加第四個變數**（Supabase service role）：
   - **Name** / **名稱**：`SUPABASE_SERVICE_ROLE_KEY`
   - **Value** / **值**：Paste the `service_role` key from Supabase **Settings → API**; server-only, never expose it publicly / 貼上 Supabase **Settings → API** 的 `service_role` 金鑰，僅伺服器端使用，勿公開。
   - Click **"Add"** / 點擊「Add」

6. **Add the fifth variable** (AI key encryption) / **添加第五個變數**（AI 金鑰加密）：
   - **Name** / **名稱**：`AI_PROVIDER_KEY_ENCRYPTION_SECRET`
   - **Value** / **值**：Set a long random server-only secret / 設定一個長且隨機的伺服器端密鑰
   - Click **"Add"** / 點擊「Add」

7. **Add the sixth variable** (AI key version) / **添加第六個變數**（AI 金鑰版本）：
   - **Name** / **名稱**：`AI_PROVIDER_KEY_ACTIVE_VERSION`
   - **Value** / **值**：`1`
   - Click **"Add"** / 點擊「Add」
