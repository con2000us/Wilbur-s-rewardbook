...
3. **依序打開並執行這些檔案**：
   ...
   - 在 GitHub 專案頁面找到檔案，點擊檔案名稱，再點右上角「Raw」
   - 全選（Ctrl+A）並複製（Ctrl+C）內容

4. **逐一貼上到 Supabase SQL Editor 並執行**：
...
### 4.4 建立 Storage 公開 bucket（大型目標圖片，選用但建議）

若你會使用 **獎勵中心 → 大型目標** 的**圖片上傳**，請在 Supabase 後台完成下列設定（僅在網頁點選即可，無須寫程式）：

1. **建立 bucket**  
   - 左側選單點 **「Storage」**  
   - 點 **「New bucket」**  
   - **Name** 填 **`goal-images`**（必須與此名稱一致）  
   - 開啟 **「Public bucket」**（公開 bucket，網頁才能顯示圖片網址）  
   - 點 **「Create」** 建立  

2. **設定公開讀取（建議）**  
   - 仍在 **Storage**，點 **「Policies」**（或進入 `goal-images` 後管理 Policies）  
   - 為 **`storage.objects`** 新增一條 **SELECT** 政策（或到 **SQL Editor** 執行專案內說明），讓 **`bucket_id = 'goal-images'`** 的物件可被讀取，以便縮圖與預覽正常顯示。  
   - 更完整的後台步驟與範例 SQL 見專案內 **`docs/STORAGE_BUCKET_SETUP.md`**。  

3. **部署環境變數（圖片上傳必填）**  
   - 上傳 API 使用 **Service Role** 寫入 Storage。請到 Supabase **Settings → API**，複製 **`service_role`** 金鑰（僅後端使用，勿寫進前端或公開 repo）。  
   - 在 **Vercel**（或本機 `.env.local`）新增環境變數：**`SUPABASE_SERVICE_ROLE_KEY`** = 上述 `service_role` 值。  
   - 若未設定，上傳可能失敗或受 Storage RLS 限制；僅編輯文字、不上傳圖片則可略過 bucket 與此變數。  
...
2. **點擊「Deploy with Vercel」按鈕**：
   - 在 README 頁面頂部
   - 或直接前往：https://vercel.com/new/clone?repository-url=https://github.com/con2000us/Wilbur-s-rewardbook&env=NEXT_PUBLIC_SUPABASE_URL,NEXT_PUBLIC_SUPABASE_ANON_KEY,SITE_PASSWORD,SUPABASE_SERVICE_ROLE_KEY,AI_PROVIDER_KEY_ENCRYPTION_SECRET,AI_PROVIDER_KEY_ACTIVE_VERSION
...
5. **添加第四個變數**（Supabase service role）：
   - **Name**：`SUPABASE_SERVICE_ROLE_KEY`
   - **Value**：貼上 Supabase **Settings → API** 的 `service_role` 金鑰，僅伺服器端使用，勿公開。
   - 點擊「Add」

6. **添加第五個變數**（AI 金鑰加密）：
   - **Name**：`AI_PROVIDER_KEY_ENCRYPTION_SECRET`
   - **Value**：設定一個長且隨機的伺服器端密鑰
   - 點擊「Add」
