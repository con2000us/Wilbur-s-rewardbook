...
- 已實作 Reset / Restart / Reassign 的基本 API 與學生頁/設定頁操作。
- 已新增並套用 `transactions.goal_id` migration，目標完成時產生的交易可精準連回 `student_goals`，Reset 可優先用此欄位清理完成獎勵、完成標記與退還交易。
- `Undo complete` 會保留原起算日；學生頁的「重設」會將起算日改為今天；「再開一輪」會建立新的 `student_goals` 實例並保留舊紀錄。
- 設定頁「停止追蹤」會將進行中的學生目標設為 `is_active=false`，保留資料並允許之後重新指派。
